...
int main(int argc, char *argv[])
{
    ...
    (void)signal(SIGALRM, alarmHandler); // Ao receber o  sinal SIGALRM, vai entrar dentro da função definida alarmHandler (foi o stor que colocou isto aqui)
    ...
    newtio.c_cc[VMIN] = 0;  // Alterou-se isto para fazer o Pooling Read -> para devolver quantos bytes é que leu
...
typedef enum {
    ...
    alarm(3); // Primeiro alarme quando se envia o primeiro SET
...
// Se o recetor não recebe nada ao fim de 3s, tem de se voltar a testar para evitar o bloqueio
// fazer um if
